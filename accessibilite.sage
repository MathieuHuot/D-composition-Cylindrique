    ...
    if Cel[0]==0:
        ...
        if L==[]: #Aucun polynome n'a de racine
            ...
            if l<k:   #Si il reste un niveau à construire, on appelle récursivement
                #Il reste à completer le dictionnaire 
                ##TODO
                Access(PolElim,PolIni,l+1,k,a+[0])
            return ()
        else:
            ...
            for i in range(len(L)):
                ...
                P=L[i][ind][2]
                r=L[i][ind][0]
                Tbis=T+[(r,P,Access(l,T,P))] #Qu'on ajoute au système triangulaire
                for j in range(lon):
                    Pol=PolIni[l-1][j]
                    pos=RechP(Pol,L[i])
                    if pos>0:
        ...
        if l<k:
            for i in range(Cel[0]):
                Access(PolElim,PolIni,l+1,k,b=a+[i])
        return ()
